...
export function Calendar({
  ...
  return (
    <div
      className={cn(
        "calendar-wrapper bg-card rounded-xl p-10 w-full max-w-[1100px] mx-auto",
        className
      )}
    >
      <div className="relative z-20 mb-6 flex items-center justify-between gap-3 h-10">
        <div className="flex items-center gap-2">
          <select
            className="h-9 rounded-md border border-border bg-card px-2 text-sm cursor-pointer"
            value={year}
            onChange={(e) => {
      ...
      <style jsx global>{`
        ...
        .calendar-wrapper .rdp-day_button:hover > span { background: hsl(var(--accent)); }
        .calendar-wrapper .rdp-selected .rdp-day_button > span,
        .calendar-wrapper .rdp-range_start .rdp-day_button > span,
        .calendar-wrapper .rdp-range_end .rdp-day_button > span { background: hsl(var(--primary)) !important; color: hsl(var(--primary-foreground)) !important; font-weight: 600 !important; box-shadow: 0 2px 10px rgba(79, 70, 229, 0.15) !important; }
        .calendar-wrapper .rdp-range_middle .rdp-day_button > span { color: hsl(var(--primary)) !important; font-weight: 600 !important; }
        .calendar-wrapper .rdp-day { position: relative; height: 48px; }
        .calendar-wrapper .rdp-range_middle::before, .calendar-wrapper .rdp-range_start::before, .calendar-wrapper .rdp-range_end::before { content: ""; position: absolute; top: 0; bottom: 0; left: 0; right: 0; background: hsl(var(--muted)); opacity: 0.5; z-index: 0; pointer-events: none; }
      ...
      <DayPicker
        ...
        classNames={{
          ...
          nav_button: cn(
            ...
            "h-9 w-9 bg-transparent p-0 hover:bg-accent border-border rounded-full transition-colors"
          ),
          table: "w-full",
          day_button: cn(
            buttonVariants({ variant: "ghost" }),
            "w-full h-full p-0 rounded-none hover:bg-muted/50 transition-none"
          ),
          ...classNames,
        }}
        {...props}
